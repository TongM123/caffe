...
void PoolingLayer<float>::Forward_cpu(const vector<Blob<float>*>& bottom,
  ...
  case PoolingParameter_PoolMethod_MAX:{
	...
	param[4] = param[1] * param[0];//pooled_fm_size 16
    param[5] = param[3] * param[2];//fm_size 20

    param[6] = param[4] * channels_;//pooled_batch_size 24
    param[7] = param[5] * channels_;//batch_size 28

    param[8] = height_ - kernel_h_;//height_kernel_h_ 32
    param[9] = width_ - kernel_w_;//width_kernel_w_ 36

    param[10] = param[8] + pad_h_;//height_kernel_h_pad_h_ 40
    param[11] = param[9] + pad_w_;//width_kernel_w_pad_w_ 44
    ...
    for (int image = 0; image < bottom[0]->num(); ++image)
    {
        for (int channel = 0; channel < num_channels; ++channel)
        {
            ...
            __asm__ __volatile__(
                ...
                //input: 
                //stack_bottom_data_ptr = bottom_data + fm_size*(layer->channels_*batch_start + channel_start)*8;//*4?*8?
                ...
                //output: 
                //stack_top_data_ptr = top_data + pooled_fm_size*(layer->channels_*batch_start + channel_start)*8;
				...
				"MOV %[debug_int2], x6 \n\t"
                "ADD x7, x5, x6\n\t"
				"ADD x5, XZR, x7, LSL #2 \n\t"        //x5=(pooled_batch_size*batch+pooled_fm_size*fm)*8
				"MOV %[debug_int3], x5 \n\t"
				"ADD %[stack_top_data_ptr], x5, %[top_data]\n\t"
				...
				"Not_Optimal:\n\t"
                    ...
                    "looppw:\n\t"
                        ...
                        //num_elements_to_do = wend - x5; 
                        "SUB num_elements_to_do, x6, x5\n\t"
                        
                        //const int index = index0 + x5; 
                        "ADD %[index], %[index0], x5\n\t"
                        
                        //const int effective_ptr = (index)*8 + input_ptr;//*4?*8
                        ...
                        "looph:\n\t"
                            ...
                            "ADD x5, %[index], num_elements_to_do\n\t"        //x5=index+num_elements_to_do
                            "MOV x6, effective_ptr\n\t"                    //x6=effective_ptr
                            "MOV x7, %[index]\n\t"                            //x7=index
				...
				*/	
				...
				://output
				[stack_bottom_data_ptr]"=&r"(stack_bottom_data_ptr), [stack_top_data_ptr]"=&r"(stack_top_data_ptr), [stack_top_mask_ptr]"=&r"(stack_top_mask_ptr), [debug_int1]"=r"(debug_int1), [debug_int2]"=r"(debug_int2),[debug_int3]"=r"(debug_int3)
				//[result]"=r"(result)
                ://input
				[param]"r"(param),
